...
$bitki_id = $_GET['bitki']; 
$query = $db->query("SELECT * FROM bitki WHERE id = '{$bitki_id}'")->fetch(PDO::FETCH_ASSOC);
if ( $query ){
    $bitki_adi = $query['bitki_adi'];
}
 ...
 <div class="slim-mainpanel">
	...
	<div class="report-summary-header">
          <div>
            <h4 class="tx-inverse mg-b-3"><?php echo $bitki_adi; ?> İçin Aylık İlaçlama Maliyetleri</h4>
          </div>
        </div>
        ...
        <div class="pagination-wrapper">
            <nav aria-label="Page navigation">
              <ul class="pagination mg-b-0">
                <li class="page-item"><a class="page-link" href="?yil=2017&bitki=<?php echo $bitki_id; ?>">2017</a></li>
                <li class="page-item active"><a class="page-link" href="?yil=2018&bitki=<?php echo $bitki_id; ?>">2018</a></li>
                <li class="page-item"><a class="page-link" href="?yil=2019&bitki=<?php echo $bitki_id; ?>">2019</a></li>
              </ul>
            </nav>
          </div>
      </div><!-- container -->
...
$query = $db->query("SELECT * FROM tuketim WHERE bitki_id = '{$bitki_id}' AND yil = '{$yil}'", PDO::FETCH_ASSOC);
if ( $query->rowCount() ){
     foreach( $query as $row ){
      $ay = $row['ay'];
          if ($ay == "1") {
